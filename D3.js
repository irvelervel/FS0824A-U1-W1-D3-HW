...
for (let i = 0; i < starWarsCharacters.length; i++) {
  // questa riga verrà eseguita 10 volte, ognuna con una i diversa (e quindi con un personaggio starWarsCharacters[i] diverso)
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  // qui dentro entrate per OGNI oggetto
  if (starWarsCharacters[i].gender === 'female') {
    // qui dentro entrate SOLO negli oggetti femminili
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  switch (starWarsCharacters[i].eye_color) {
    // può essere yellow, può essere blue, può essere red etc.
    case 'yellow':
      // faccio cose
...
while (counter < starWarsCharacters.length) {
  // per ogni personaggio dell'array addizioniamo la sua massa al totale precedentemente accumulato nella variabile crewMass
  ...
  // CICLO 1 (counter vale 0)
  // crewMass era 0
  // crewMass ora vale 0 + il peso del primo personaggio (277)

  // CICLO 2 (counter vale 1)
  // crewMass era 277
  // crewMass ora vale 277 + il peso del secondo personaggio (75)

  // CICLO 3 (counter vale 2)
  // crewMass era 352
  // crewMass ora vale 352 + il peso del terzo personaggio (32)
}
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  // riassegniamo un nuovo valore a crewMass2 -> le sommiamo al valore precedente la massa del personaggio corrente
  crewMass2 = crewMass2 + starWarsCharacters[i].mass
}
...
for (let i = 0; i < femaleCharacters.length; i++) {
  ...
  for (let j = 0; j < charactersNames.length; j++) {
    if (femaleCharacters[i].name === charactersNames[j]) {
      // abbiamo trovato il corrispondente nell'array charactersNames
      // eliminamo l'elemento trovato dall'array charactersNames
